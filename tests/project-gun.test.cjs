...
assert.equal(graphics.maxDpr, 1.5);
assert.deepEqual(Object.keys(graphics.presets), ["low", "medium", "high"]);
assert.equal(graphics.presets.medium.pixelRatio, 1.2);
...
assert.match(source, /medium:\{pixelRatio:1\.2,samples:1,fxaa:true/);
assert.match(source, /high:\{pixelRatio:1\.5,samples:2,fxaa:false/);
