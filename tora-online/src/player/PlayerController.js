...
export class PlayerController extends Entity {
  constructor(visual, input, navigation) {
    ...
    this.grounded = true; this.verticalVelocity = 0; this.buffs = { guard: 0, rage: 0 }; this.speedRatio = 0;
  ...
  setDestination(point, stopRange = .2) { this.destination = this.navigation.findReachable(this.position, point); this.stopRange = stopRange; }
  cancelDestination() { this.destination = null; }
  dash(direction) {
  ...
  update(dt, camera) {
    ...
    if (manual) this.destination = null;
    else if (this.destination) {
      direction = this.destination.subtract(this.position); direction.y = 0;
      if (direction.length() <= this.stopRange) { this.destination = null; direction.setAll(0); }
    }
