...
export class CombatSystem {
  constructor(scene,player,animator,entities,callbacks={}){
    ...
    this.effects=this.#createEffectPool();this.effectCursor=0;
    this.playerCombat=new PlayerCombat(player,animator,entities,this.skills,{...callbacks,onCast:(skill,from,to)=>{this.#effect(from,skill.color,skill.aoe?2.5:1.2);callbacks.onCast?.(skill,from,to);},onImpact:(skill,from,to)=>{if(to)this.#effect(to,skill.color,skill.slot===9?2:1);callbacks.onImpact?.(skill,from,to);}});
  ...
  update(dt){this.skills.update(dt);this.playerCombat.update(dt);if(this.playerHitTimer>0){this.playerHitTimer-=dt;if(this.playerHitTimer<=0&&this.player.alive)this.player.state="idle";}for(const effect of this.effects){if(effect.metadata.life>0){effect.metadata.life-=dt;effect.scaling.scaleInPlace(1+dt*2.4);effect.visibility=Math.max(0,effect.metadata.life/.45);if(effect.metadata.life<=0)effect.setEnabled(false);}}if(!this.player.alive){this.playerRespawn-=dt;if(this.playerRespawn<=0)this.#respawnPlayer();}}
  damagePlayer(result){this.callbacks.onDamage?.(this.player,result,null);if(this.player.alive){this.playerHitTimer=.2;this.player.state="hit";}else if(this.playerRespawn<=0){this.playerRespawn=4;this.player.state="dead";this.callbacks.onStatus?.("Savaşçı düştü. Kadim bağ seni geri çağırıyor…");}}
  #respawnPlayer(){this.player.health=this.player.maxHealth;this.player.mana=this.player.maxMana;this.player.alive=true;this.player.position.set(0,0,-18);this.player.state="idle";this.playerRespawn=0;}
