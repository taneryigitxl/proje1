...
export class CombatSystem {
  constructor(scene, player, animator, entities, callbacks = {}, stats = null) {
    ...
    this.playerCombat = new PlayerCombat(player, animator, entities, this.skills, {
      ...
      onCast: (skill, from, to) => {
        this.#effect(from, skill.color, skill.aoe ? 2.5 : 1.2);
        this.#burst(from, skill.color, skill.aoe ? 1.6 : 1);
        callbacks.onCast?.(skill, from, to);
      },
      onImpact: (skill, from, to) => {
        this.#effect(to || from, skill.color, skill.slot === 9 ? 2 : skill.aoe ? 2.5 : 1);
        this.#burst(to || from, skill.color, skill.slot === 9 ? 1.8 : skill.aoe ? 1.5 : 1.1);
  ...
  update(dt) {
    ...
    for (const effect of this.effects) {
      if (effect.metadata.life > 0) {
        ...
        effect.scaling.scaleInPlace(1 + dt * 2.4);
        effect.visibility = Math.max(0, effect.metadata.life / .45);
    ...
    for (const burst of this.burstPool) {
      if (burst.metadata.life > 0) {
        ...
        const t = burst.metadata.life / .35;
        burst.scaling.setAll(burst.metadata.baseScale * (1.2 - t * .4));
  ...
  #createEffectPool() {
    const material = new BABYLON.StandardMaterial("combat-effect-material", this.scene);
    material.emissiveColor = new BABYLON.Color3(.65, .2, 1);
    material.alpha = .8;
    material.disableLighting = true;
    return Array.from({ length: 12 }, (_, i) => {
      const mesh = BABYLON.MeshBuilder.CreateTorus(`combat-effect-${i}`, { diameter: 2, thickness: .06, tessellation: 24 }, this.scene);
  ...
  #createBurstPool() {
    return Array.from({ length: 10 }, (_, i) => {
      ...
      material.alpha = .55;
      material.disableLighting = true;
      const mesh = BABYLON.MeshBuilder.CreateSphere(`combat-burst-${i}`, { diameter: .85, segments: 8 }, this.scene);
      mesh.material = material;
      mesh.isPickable = false;
      mesh.setEnabled(false);
  ...
  #createSwordTrail() {
    ...
    try {
      const trail = new BABYLON.TrailMesh("player-sword-trail", generator, this.scene, .14, 24, false);
      const material = new BABYLON.StandardMaterial("sword-trail-material", this.scene);
      material.emissiveColor = new BABYLON.Color3(.7, .2, 1);
