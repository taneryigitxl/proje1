...
export class SkillSystem {
  ...
  canUse(skill, player) { if (!skill) return { ok: false, reason: "Yetenek bulunamadı." }; if (!player.alive) return { ok: false, reason: "Karakter savaşamaz." }; if (this.remaining(skill.slot) > 0) return { ok: false, reason: "Yetenek bekleme süresinde." }; if (player.mana < skill.mana) return { ok: false, reason: "Yeterli mana yok." }; return { ok: true }; }
