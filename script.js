...
function drawPlatform(p,theme){
  const styles={meadow:{top:"#788b63",mid:"#59634e",deep:"#30382f",line:"#242a24"},ruins:{top:"#c29a67",mid:"#8d6844",deep:"#493425",line:"#59402c"},mine:{top:"#6a5d62",mid:"#423a40",deep:"#211d23",line:"#17141a"},storm:{top:"#63838a",mid:"#385962",deep:"#1d333b",line:"#172a31"},boss:{top:"#4b4242",mid:"#282426",deep:"#100d10",line:"#080709"}},s=styles[theme]||styles.meadow;
...
function drawPlayer(p){
  ...
  ctx.restore();
  if(p.type==="nita"&&p.invisible>0){const label=`${p.invisible.toFixed(1)} sn`,cx=renderX+p.w/2,cy=renderY-(state.boss?34:18),pulse=.82+Math.sin(performance.now()*.012)*.18;ctx.save();ctx.font='800 10px "Manrope"';ctx.textAlign="center";ctx.textBaseline="middle";const width=Math.max(42,ctx.measureText(label).width+16);ctx.shadowColor="#54d8e8";ctx.shadowBlur=12*pulse;ctx.fillStyle="rgba(8,18,25,.78)";ctx.beginPath();ctx.roundRect(cx-width/2,cy-10,width,20,10);ctx.fill();ctx.shadowBlur=0;ctx.strokeStyle=`rgba(84,216,232,${.45+.25*pulse})`;ctx.lineWidth=1;ctx.stroke();ctx.fillStyle="#c9fbff";ctx.fillText(label,cx,cy+.5);ctx.restore();}
