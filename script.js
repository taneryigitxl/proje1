...
function draw(){
  const rect=canvas.getBoundingClientRect(),mobileFill=matchMedia("(max-width:950px) and (orientation:landscape)").matches,scaleX=rect.width/WORLD.width,scaleY=rect.height/WORLD.height,scale=mobileFill?null:Math.min(scaleX,scaleY),ox=mobileFill?0:(rect.width-WORLD.width*scale)/2,oy=mobileFill?0:(rect.height-WORLD.height*scale)/2,level=levels[state.level]||levels[0];ctx.fillStyle=COLORS.dark;ctx.fillRect(0,0,rect.width,rect.height);ctx.save();ctx.translate(ox,oy);ctx.scale(mobileFill?scaleX:scale,mobileFill?scaleY:scale);drawBackground(level);
